...
dfc = dfo.copy()
dfc = dfc.loc[
    (dfc["value"] >= dfc["value"].quantile(0.025))
...
def draw_bar_plot():
    ...
    # Use sns.catplot().
    graph = sns.catplot(
        data=df1, kind="bar", x="year", y="value", hue="month_long", hue_order=meses
    )
...
def draw_box_plot():
    ...
    ax2 = sns.boxplot(data=df2, x="month_short", y="value", order=mesitos, ax=ax2)
